								...
								<div v-if="each['device_type'] == 3 || each['device_type'] == 4|| each['device_type'] == 5" style="width: 100%; display: flex;flex-direction: column;align-items: center;">
									...
									<div v-for="(data_each,data_index) in each.datastreams" :obj="data_each.id" style="width: 100%; display: flex;flex-direction: column;align-items: center;">
										...
										<div v-if="each['device_type'] == 5" class="flex" style="white-space: pre-wrap; text-align:center;vertical-align:middel;">
											<button v-if="data_each.value.st[1] != null" class="btn btn-secondary" style="background-color: #532222;height: 50rpx;font-size: 24rpx;" @click="data_each.value.st = [data_each.value.st[0]]; set_onenet_http(key.substr(1,), 'st', data_each.value.st.join());delay_fresh(500);">关指令查</button>
											<button v-else class="btn btn-secondary" style="background-color:darkslateblue;height: 50rpx;font-size: 24rpx;" @click="data_each.value.st = [data_each.value.st[0]==''?0:data_each.value.st[0], 1.0, 0]; set_onenet_http(key.substr(1,), 'st', data_each.value.st.join());delay_fresh(500);">开指令查</button>
											<span v-html="'&nbsp;'"></span>
											<button v-if="data_each.value.st[1] != null && data_each.value.st[2]!=1" class="btn btn-secondary" style="background-color:darkslateblue;height: 50rpx;font-size: 24rpx;" @click="data_each.value.st[2] = 1; set_onenet_http(key.substr(1,), 'st', data_each.value.st.join()); delay_fresh(500);">下发定位指令</button>
											...
											<span v-html="'&nbsp;'"></span>
											<button v-if="data_each.value.m_conf[0] == '' || data_each.value.m_conf[0] == null || data_each.value.m_conf[0] == '0'" class="btn btn-secondary" style="background-color:darkslateblue;height: 50rpx;font-size: 24rpx;" @click="data_each.value.m_conf = ['1', data_each.value.m_conf[0]!='1'?'0':data_each.value.m_conf[1], data_each.value.m_conf[2]!=null?data_each.value.m_conf[2]:'0']; set_onenet_http(key.substr(1,), 'm_conf', data_each.value.m_conf.join());delay_fresh(500);">开启震动开机</button>
											<button v-else class="btn btn-secondary" style="background-color: #532222;height: 50rpx;font-size: 24rpx;" @click="data_each.value.m_conf = ['0', '0', data_each.value.m_conf[2]!=null?data_each.value.m_conf[2]:'0']; set_onenet_http(key.substr(1,), 'm_conf', data_each.value.m_conf.join());delay_fresh(500);">关闭震动开机</button>
										</div>
